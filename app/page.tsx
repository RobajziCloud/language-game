...
const BANK: Pack[] = [
  {
    id: "s1",
    english: ["I", "have", "never", "seen", "such", "a", "beautiful", "forest", "before"],
    tokens: [
      { w: "I", pos: "pronoun", meaning: "the speaker" },
      { w: "have", pos: "auxiliary verb", meaning: "helps form present perfect" },
      { w: "never", pos: "adverb", meaning: "at no time in the past" },
      { w: "seen", pos: "past participle", meaning: "perceived with the eyes" },
      { w: "such", pos: "determiner", meaning: "of this type or degree" },
      { w: "a", pos: "article", meaning: "one; any single" },
      { w: "beautiful", pos: "adjective", meaning: "pleasant to see" },
      { w: "forest", pos: "noun", meaning: "large area covered with trees" },
      { w: "before", pos: "adverb", meaning: "at any earlier time" },
    ],
    explanation: "V předpřítomném čase stojí příslovce (never) před příčestím (seen): have + adverb + past participle.",
  },
  {
    id: "s2",
    english: ["She", "will", "probably", "call", "you", "this", "afternoon"],
    tokens: [
      { w: "She", pos: "pronoun", meaning: "a female person" },
      { w: "will", pos: "modal verb", meaning: "future marker" },
      { w: "probably", pos: "adverb", meaning: "likely" },
      { w: "call", pos: "verb", meaning: "to phone" },
      { w: "you", pos: "pronoun", meaning: "the person addressed" },
      { w: "this", pos: "determiner", meaning: "refers to the present day" },
      { w: "afternoon", pos: "noun", meaning: "time between noon and evening" },
    ],
    explanation: "Příslovce pravděpodobnosti obvykle stojí za 'will' a před významovým slovesem: will probably call.",
  },
  {
    id: "s3",
    english: ["There", "are", "too", "many", "cars", "on", "this", "street"],
    tokens: [
      { w: "There", pos: "dummy subject", meaning: "introduces existence" },
      { w: "are", pos: "verb", meaning: "present of 'be' (plural)" },
      { w: "too", pos: "adverb", meaning: "excessively" },
      { w: "many", pos: "determiner", meaning: "a large number of countable things" },
      { w: "cars", pos: "noun", meaning: "vehicles" },
      { w: "on", pos: "preposition", meaning: "in contact with and supported by" },
      { w: "this", pos: "determiner", meaning: "near in space or time" },
      { w: "street", pos: "noun", meaning: "road in a city" },
    ],
    explanation: "U počitatelných podstatných jmen používáme 'many', u nepočitatelných 'much'.",
  },
];
...
export default function Page() {
  ...
  const [packIndex, setPackIndex] = useState(() => Math.floor(Math.random() * BANK.length));
  const pack = BANK[packIndex];
  const [pool, setPool] = useState<string[]>(() => shuffle(pack.english));
  const [slots, setSlots] = useState<Slot[]>(() => pack.english.map((_, i) => ({ id: `s-${i}`, token: null })));
  const [verdict, setVerdict] = useState<Verdict[]>(() => Array(pack.english.length).fill(null));
  ...
  const canVerify = useMemo(() => slots.every((s) => s.token !== null), [slots]);

  const onNextRound = () => {
    setShowExplain(false);
    setActiveToken(null);
    const nextIndex = (packIndex + 1) % BANK.length;
    setPackIndex(nextIndex);
    const next = BANK[nextIndex];
    setSlots(next.english.map((_, i) => ({ id: `s-${round}-${i}`, token: null })));
    setPool([]);
    setRound((r) => r + 1);
    setTimeout(() => setPool(shuffle(next.english)), 350);
    setVerdict(Array(next.english.length).fill(null));
  };
      ...
      <div className="relative mx-auto max-w-6xl px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3 text-xs">
            <Stat label="Kola" value={stats.rounds} />
            <Stat label="Úspěšnost" value={stats.totalTokens ? Math.round((stats.correctTokens / stats.totalTokens) * 100) + "%" : "–"} />
          </div>
        </div>
              ...
              <Card className="bg-white/5 border-white/10 backdrop-blur-xl">
                ...
                <CardContent>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {pack.tokens.map((t) => (
                      <WordInfo key={t.w} token={t} open={activeToken === t.w} onToggle={() => setActiveToken((cur) => (cur === t.w ? null : t.w))} />
                    ))}
                  </div>
...
function WordInfo({ token, open, onToggle }: { token: Token; open: boolean; onToggle: () => void }) {
  const [hover, setHover] = useState(false);
  return (
    <div className="relative" onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}>
      <button className="px-3 py-2 rounded-xl bg-white/10 hover:bg-white/20 ring-1 ring-white/15 text-sm text-zinc-100 shadow" onClick={onToggle}>
        {token.w}
